...
const Companies = () => {
    ...
    useEffect(() => {
      const getComapany = async () => {
        try {
          console.log('Fetching job seekers...');
          console.log('Token:', token);
  
          const response = await fetch("http://localhost:8080/jobseeker/getAlEmployers", {
            headers: {
          ...
          jobseekers.map((item) => (
            <div key={item.id} className="card">
              <div className="card-body">
                ...
                <h6 className="card-subtitle mb-2 text-muted">Skills - {item.address}</h6>
